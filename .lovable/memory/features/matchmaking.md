...
## Check / pontuação
RPC `create_matchmaking_check(_target_id, _description, _meeting_date)` (SECURITY DEFINER):
1. Cria registro em `gente_em_acao` (membro→partner_id; convidado→guest_name) → 25 pts pela mecânica existente.
...
Bônus de +10 pts somado em `calculate_monthly_points_for_team` (termo `matchmaking_count * 10`), preservando todos os demais cálculos.
