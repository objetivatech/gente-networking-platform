-- Fix ambiguous user_id in get_members_health_scores
...
BEGIN
  ...
  SELECT ur.role INTO caller_role FROM user_roles ur WHERE ur.user_id = auth.uid() LIMIT 1;
  ...
  team_of AS (
    SELECT DISTINCT ON (tm.user_id) tm.user_id AS uid, tm.team_id
    FROM team_members tm
    ORDER BY tm.user_id, tm.joined_at ASC
  ),
  ga AS (
    SELECT g.user_id AS uid, COUNT(*)::int AS cnt, MAX(g.meeting_date)::timestamptz AS last_at
    FROM gente_em_acao g WHERE g.meeting_date >= since_date GROUP BY g.user_id
  ),
  refs AS (
    SELECT r.from_user_id AS uid, COUNT(*)::int AS cnt, MAX(r.created_at) AS last_at
    FROM referrals r WHERE r.created_at >= since_ts GROUP BY r.from_user_id
  ),
  att AS (
    SELECT a.user_id AS uid, COUNT(*)::int AS cnt, MAX(m.meeting_date)::timestamptz AS last_at
    FROM attendances a JOIN meetings m ON m.id = a.meeting_id
    WHERE m.meeting_date >= since_date GROUP BY a.user_id
  ),
  test AS (
    SELECT t.from_user_id AS uid, COUNT(*)::int AS cnt, MAX(t.created_at) AS last_at
    FROM testimonials t WHERE t.created_at >= since_ts GROUP BY t.from_user_id
  ),
  council AS (
    SELECT cr.user_id AS uid, COUNT(*)::int AS cnt, MAX(cr.created_at) AS last_at
    FROM council_replies cr WHERE cr.created_at >= since_ts GROUP BY cr.user_id
  ),
  bc AS (
    SELECT b.user_id AS uid, COUNT(*)::int AS cnt, MAX(b.created_at) AS last_at
  ...
  SELECT
    ...
    END AS health_level
  FROM members mem
  JOIN profiles p ON p.id = mem.uid AND p.is_active = true
  LEFT JOIN team_of tof ON tof.uid = mem.uid
  LEFT JOIN teams t ON t.id = tof.team_id
  LEFT JOIN ga ON ga.uid = mem.uid
  LEFT JOIN refs ON refs.uid = mem.uid
  LEFT JOIN att ON att.uid = mem.uid
  LEFT JOIN test ON test.uid = mem.uid
  LEFT JOIN council ON council.uid = mem.uid
  LEFT JOIN bc ON bc.uid = mem.uid
