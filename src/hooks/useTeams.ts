...
export interface TeamMember {
  ...
  joined_at: string;
  profile: { full_name: string; company: string | null; avatar_url: string | null; rank: 'iniciante' | 'bronze' | 'prata' | 'ouro' | 'diamante' };
}
  ...
  const { data: teams, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return teamsData.map(team => ({
        ...team,
        members: membersData?.filter(m => m.team_id === team.id).map(m => ({ ...m, profile: profiles[m.user_id] })) || []
      })) as Team[];
    },
  });
